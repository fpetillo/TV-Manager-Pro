...
window.editShowPreferences=async function(id,name){
  ...
  const read=buildShowPreferences(form,show.show,profiles.results||[]);const error=document.createElement('p');error.setAttribute('role','alert');form.append(error);const defaults=document.createElement('button');defaults.type='button';defaults.textContent='Save These as New-Show Defaults';defaults.onclick=async()=>{defaults.disabled=true;try{const r=await fetch('/api/shows/defaults',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(read())}),d=await r.json();if(!r.ok)throw new Error(d.error||'Save failed');error.textContent='Defaults saved for future shows. Existing shows are unchanged.';}catch(ex){error.textContent=ex.message;}finally{defaults.disabled=false;}};form.append(defaults);
