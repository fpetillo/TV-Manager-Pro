...
async function refreshSeasonCountsDetailed(){
  try{
    const sd=await jsonFetch(`/api/shows/${showId}/episodes`,{timeout:3000});
    if(!sd.seasons || !sd.seasons.length)return;
    const current=seasonFilter.value;
    seasonFilter.innerHTML='<option value="">All seasons</option>'+(sd.seasons||[]).map(s=>`<option value="${s.season}">${Number(s.season)===0?'Specials':'Season '+s.season} (${s.considered_count??s.episode_count??0}${s.ignored_count?`, ${s.ignored_count} ignored`:''})</option>`).join('');
...
async function loadShowCountsFast(){
  try{
    const d=await jsonFetch(`/api/shows/${showId}`,{timeout:3000});
    const sh=d.show||{};
...
async function loadEpisodes(){
  ...
  try{
    const d=await jsonFetch(`/api/shows/${showId}/episodes`,{timeout:10000});
    if(generation!==seasonGeneration)return;
    if(d.error||d.degraded)throw new Error(d.error||'Season list is temporarily unavailable.');
    const seasonCounts=new Map((d.seasons||[]).map(s=>[Number(s.season),Number(s.episode_count||0)]));
    const seasonDownloaded=new Map((d.seasons||[]).map(s=>[Number(s.season),Number(s.with_files||0)]));
    ...
    episodeBody.innerHTML=seasons.map(season=>`<tr class="season-folder-row"><td colspan="8"><details class="season-folder" data-season="${season}"><summary><span class="season-folder-icon" aria-hidden="true">▸</span> ${season===0?'Specials (S00)':'Season '+String(season).padStart(2,'0')} <span class="season-episode-count">${seasonDownloaded.get(season).toLocaleString()} of ${seasonCounts.get(season).toLocaleString()} ${seasonCounts.get(season)===1?'episode':'episodes'} downloaded</span> <span class="muted">Click to expand or collapse</span></summary><div class="season-message" role="status"></div><div class="table-wrap"><table class="compact-table episode-drill-table"><thead><tr><th>Select</th><th>Ep</th><th>Title</th><th>Air date</th><th>Status / Rules</th><th>Quality</th><th>File / Path</th><th>Action</th></tr></thead><tbody></tbody></table></div></details></td></tr>`).join('')||'<tr><td colspan="8">No seasons available.</td></tr>';
