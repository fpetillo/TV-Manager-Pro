...
let sort='show',direction='asc',offset=0,total=0;
const numericDefaultDesc=new Set(['downloads','download_percent','size','active']);
function fmtDate(v){
  if(!v)return '';
...
async function load(){
  ...
  try{
    ...
    showQueueSummary.textContent=total?`Showing ${offset+1}-${offset+rows.length} of ${Number(total).toLocaleString()} shows · sorted by ${sort} ${direction}`:'0 shows';
    showQueueBody.innerHTML=rows.length?rows.map(r=>{
      ...
      return `<tr data-show-id="${esc(r.id)}"><td>${esc(fmtDate(r.next_ep))}</td><td>${esc(fmtDate(r.prev_ep))}</td><td><a href="/show/${r.id}"><strong>${esc(r.name)}</strong></a></td><td>${esc(r.network||'')}</td><td><span class="quality-badge">${esc(r.quality||'HD')}</span></td><td class="downloads-cell" title="${downloaded} downloaded, ${missing} missing, ${totalEpisodes} total"><div class="download-meter ${meterClass}"><span style="width:${Math.max(0,Math.min(100,pp))}%"></span><strong>${downloaded}/${totalEpisodes}</strong></div></td><td class="num-cell">${esc(fmtSize(r.size_bytes))}</td><td class="center-cell">${r.active_flag?'✓':'—'}</td><td><span class="status-pill ${esc(r.status)}">${esc(r.status)}</span></td></tr>`
