...
| Downloads | Client handoff and queue monitoring | Download Center | Partial | engine.py: handoff adapters exist; polling coverage differs by client. Credentials and real clients required for end-to-end checks. |
...
| Notifications | Native notification services | Webhooks | Partial | Generic webhooks exist; they do not replace every native SickChill notifier. See upstream inventory. |
