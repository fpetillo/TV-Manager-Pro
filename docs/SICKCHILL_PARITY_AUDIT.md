# SickChill coverage audit — v18.8.0, reviewed 2026-09-15

Full parity is not achieved. Complete means the named local workflow exists; it does not certify every external integration or an exhaustive match to every upstream option. Partial identifies remaining behavior or integration coverage. The inventory has 15 locally complete and 17 partial families; these are not percentages of parity.

v18.8.0 corrects imported-provider enablement, adds editable connections and capability-aware searches, introduces reviewed existing-show source/order migration, adds Deluge daemon and put.io, and connects native clients to the main test/poll workflow. v18.7.0 delivered restore, archive/script processing, durable jobs and handoff reconciliation, calendar subscriptions and packaging corrections.

The previous audit incorrectly described AniDB as a primary show indexer. The inspected upstream ShowIndexer registers TVDB; AniDB is used for anime mapping, groups and MyList. Those auxiliary AniDB functions remain missing. TVmaze is a TVDB lookup/search helper in the inspected source, not a second registered primary indexer. This distinction avoids treating filenames as proof of an active feature.

Sources: [official feature list](https://sickchill.github.io/), [show settings](https://github.com/SickChill/sickchill/wiki/Show-settings-explained), [settings](https://github.com/SickChill/sickchill/wiki/Settings-explained), [registered indexer](https://github.com/SickChill/sickchill/blob/master/sickchill/show/indexers/handler.py), [anime integration](https://github.com/SickChill/sickchill/blob/master/sickchill/adba/aniDBAbstracter.py). Upstream inventory: `e1f8475ded8dd77662fad3ba9488740133ca8ce1`.
...
| Search | Manual and backlog episode search | Show Detail / Missing | Partial | engine.py, show_missing_search.py: aired-only manual/backlog/bulk search, per-episode locks, duplicate handoff checks and safe bulk stop. Newznab capability/auth tests passed against configured NZBFinder; full live search-to-import acceptance remains open. |
...
| Search | Anime and automatic XEM mapping | Advanced | Partial | scene_sync.py: cached XEM refresh after metadata refresh, manual override precedence and reverse matching. Live XEM returned HTTP 403 in this environment; all stored aliases and scene destinations are searched; split scene episodes are blocked from single-file retrieval. Anime group rules, AniDB MyList and split-file assembly remain missing. |
| Search | Native SickChill provider roster | Providers | Partial | indexer_client.py and provider_manager.py: imported on/off and daily/backlog flags, editable connections, capabilities, text fallback, pagination, XML errors and rate-limit handling. Site-specific login/cookies/scraping still require a Torznab bridge or a native adapter. |
| Downloads | Client handoff and queue monitoring | Download Center | Partial | engine.py, native_downloaders.py: SABnzbd, NZBGet, qBittorrent, Transmission, Deluge Web, uTorrent, rTorrent HTTP XML-RPC and Download Station handoff/polling. Real blackhole descriptors and durable uncertain-handoff review are implemented. v18.8.0 connects all native adapters to the main test/poll loop and adds Deluge daemon and put.io. MLDonkey remains absent; live client acceptance remains open. |
...
| Metadata | TMDb refresh and language | Show Settings | Partial | metadata_service.py: per-show language and default new-episode statuses; requires TMDb configuration. A failed season fetch now fails the refresh before any database update; source/order changes are checked again before commit. |
| Metadata | TVDB metadata and source migration | Metadata Sources / Add Show | Partial | tvdb_client.py implements TVDB v4 search and refresh for new shows. metadata_migration.py adds reviewed existing-show TMDb/TVDB source changes with preserved episode IDs/history/files. Live TVDB credentials required. AniDB is an auxiliary anime/MyList integration upstream, not a primary show indexer. |
| Metadata | DVD episode ordering | TVDB Add Show | Partial | New TVDB shows can choose aired/DVD order. Existing libraries have signed preview, manual episode matching, atomic order changes, active-download guards and an audit record; the browser fixture passed. Live TVDB validation is outstanding. |
