...
def test_database_safety_uses_sqlite_backup_api_and_redacts_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(database_safety, "BACKUPS", tmp_path / "backups")
    monkeypatch.setattr(database_safety, "SAFE_BACKUPS", tmp_path / "backups" / "safe")
    monkeypatch.setattr(database_safety, "MANIFEST", tmp_path / "backups" / "manifest.json")
