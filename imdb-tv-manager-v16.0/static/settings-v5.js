...
async function loadScheduler(){const [r,c]=await Promise.all([fetch("/api/scheduler"),fetch("/api/tvmanager/config")]),d=await r.json(),cfg=await c.json();automation.checked=d.automation_enabled;autoGrab.checked=cfg.auto_grab;recentDays.value=cfg.recent_days;maxSearches.value=cfg.max_searches_per_run;automationNotice.className=d.automation_enabled?"notice good":"notice warn";automationNotice.textContent=d.automation_enabled?"Automation is armed. Scheduled jobs may search and process automatically.":"Automation is paused. Manual actions remain available.";jobs.innerHTML=d.jobs.map(j=>`<div class="scheduler-row"><div><strong>${esc(j.name.replaceAll("_"," "))}</strong><div class="muted">${esc(j.last_status||"Never run")}${j.last_run?" • "+esc(j.last_run):""}</div></div><label class="switchline"><input class="checkbox jobtoggle" data-name="${esc(j.name)}" type="checkbox" ${j.enabled?"checked":""}> Enabled</label><input class="jobinterval" data-name="${esc(j.name)}" type="number" min="1" value="${j.interval_minutes}"><button class="secondary runjob" data-name="${esc(j.name)}">Run now</button></div>`).join("");document.querySelectorAll(".jobtoggle").forEach(x=>x.onchange=()=>saveJob(x.dataset.name,{enabled:x.checked}));document.querySelectorAll(".jobinterval").forEach(x=>x.onchange=()=>saveJob(x.dataset.name,{interval:+x.value}));document.querySelectorAll(".runjob").forEach(x=>x.onclick=async()=>{x.disabled=true;x.textContent="Running…";await fetch(`/api/scheduler/jobs/${x.dataset.name}/run`,{method:"POST"});x.disabled=false;x.textContent="Run now";loadScheduler()})}
